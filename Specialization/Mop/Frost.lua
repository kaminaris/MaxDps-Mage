...
function Frost:precombat()
    ...
    if (MaxDps:CheckSpellUsable(classtable.FrostArmor, 'FrostArmor')) and cooldown[classtable.FrostArmor].ready and not UnitAffectingCombat('player') then
...
function Frost:callaction()
    ...
    if (MaxDps:CheckSpellUsable(classtable.FrostBomb, 'FrostBomb')) and (not debuff[classtable.FrostBombDeBuff].up) and cooldown[classtable.FrostBomb].ready then
