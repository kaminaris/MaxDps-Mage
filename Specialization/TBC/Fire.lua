...
local function ClearCDs()
    MaxDps:GlowCooldown(classtable.Combustion, false)
end

function Fire:Single()
    -- Use Combustion when ready
    if (MaxDps:CheckSpellUsable(classtable.Combustion, 'Combustion')) and cooldown[classtable.Combustion].ready then
        ...
        MaxDps:GlowCooldown(classtable.Combustion, true)
    end
    -- Only use Scorch to maintain 5 stacks of Improved Scorch if talented, otherwise spam Fireball on targets lasting 4+ casts
    if ttd > 22 then
        -- Apply or refresh 5 stacks of Improved Scorch before spamming Fireball
        ...
        if (MaxDps:CheckSpellUsable(classtable.Scorch, 'Scorch')) and (talents[classtable.ImprovedScorch]) and cooldown[classtable.Scorch].ready then
            if not setSpell then setSpell = classtable.Scorch end
        end
    end
    -- Primary rotation: Fireball spam on targets lasting 4+ casts
...
function Fire:AoE()
    if (MaxDps:CheckSpellUsable(classtable.ArcaneExplosion, 'Arcane Explosion')) and talents[classtable.ArcaneConcentration] and (( LibRangeCheck and LibRangeCheck:GetRange ( 'target', true ) or 0 ) <10) and cooldown[classtable.ArcaneExplosion].ready then
    ...
    if (MaxDps:CheckSpellUsable(classtable.DragonsBreath, 'Dragons Breath')) and (( LibRangeCheck and LibRangeCheck:GetRange ( 'target', true ) or 0 ) <10) and cooldown[classtable.DragonsBreath].ready then
...
function Mage:Fire()
    ...
    classtable.ArcaneConcentration = 11213
    ...
    classtable.BlastWave = 33933
    classtable.DragonsBreath = 33043
